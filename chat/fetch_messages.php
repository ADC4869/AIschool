...
try {
    if ($role === 'giaovien') {
        // Lấy tất cả học sinh trong lớp mà giáo viên chủ nhiệm hoặc dạy
        ...
        $query_teachers = "
            SELECT m.id AS chat_id, u.fullname AS teacher_name, s.subject_name, 
                COALESCE(m.message, 'Chưa có tin nhắn') AS message, 
                COALESCE(m.created_at, NOW()) AS created_at, 
                COALESCE(m.is_read, 1) AS is_read
            FROM teachers t
        ...
        $query_students = "
            SELECT m.id AS chat_id, u.fullname AS name, 
                CASE 
    ...
    http_response_code(500); // Trả về mã lỗi 500 cho lỗi server
